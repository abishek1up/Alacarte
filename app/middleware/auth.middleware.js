...
module.exports = {
  authValidate: async (req, res, next) => {
    ...
    try {
      if (!authHeader) {
        return res.status(401).json({ Message: "Authorization Header not present, Please login." });
      }
      const token = authHeader.split(" ")[1];
      if (!token) {
        return res.status(401).json({ Message: "Token Not Present, Please login." });
      }
      ...
      const result = await axios
        ...
        .catch(function (error) {
          let err = new Error(error.response.data.error);
